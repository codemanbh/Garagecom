...
class UserService {
  ...
  static Future<Map<String, dynamic>> getUserProfile() async {
    try {
      ...
      if (response['succeeded'] == true && 
        ...
        throw Exception('Failed to load user profile: ${response['message'] ?? "Unknown error"}');
      }
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> updateUserProfile(Map<String, dynamic> userData) async {
    try {
      final response = await ApiHelper.post('api/Profile/UpdateUserDetails', userData);
  ...
  static Future<Map<String, dynamic>> getUserCars() async {
    try {
      ...
      if (response['succeeded'] == true && 
        ...
        throw Exception('Failed to load user cars: ${response['message'] ?? "Unknown error"}');
      }
    } catch (e) {
  ...
  // Add a new car
  static Future<Map<String, dynamic>> addCar(Map<String, dynamic> carData) async {
    try {
      // Simplify the data structure to match what the API expects
      final simplifiedData = {
        'carModelID': carData['carModel']['carModelID'],
        'year': carData['year'],
      ...
      if (response['succeeded'] == true) {
        ...
        throw Exception('Failed to add car: ${response['message'] ?? "Unknown error"}');
      }
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> updateCar(Map<String, dynamic> carData) async {
    try {
      ...
      if (response['succeeded'] == true) {
        ...
        throw Exception('Failed to update car: ${response['message'] ?? "Unknown error"}');
      }
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> deleteCar(int carId) async {
    try {
      final response = await ApiHelper.post('api/Cars/DeleteCar', {'carID': carId});
      
      if (response['succeeded'] == true) {
        return response;
      } else {
        throw Exception('Failed to delete car: ${response['message'] ?? "Unknown error"}');
      }
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> uploadProfilePicture(File imageFile) async {
    try {
      final response = await ApiHelper.uploadImage(imageFile, 'api/Users/UploadProfilePicture');
      
      if (response['succeeded'] == true) {
        return response;
      } else {
        throw Exception('Failed to upload profile picture: ${response['message'] ?? "Unknown error"}');
      }
    } catch (e) {
